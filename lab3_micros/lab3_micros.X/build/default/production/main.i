...
loop:
    btfsc ((INTCON) and 07Fh), 2 ;si la bandera es 1 se increenta el contador (puertoB)
    incf PORTB

    btfsc ((INTCON) and 07Fh), 2 ;si la bandera es 1 se reinicia el tmr0
    call reiniciar_tmr0
...
config_io:
    ...
    bsf TRISA, 0 ;puerto ((PORTA) and 07Fh), 0 como entrada
    bsf TRISA, 1 ;puerto ((PORTA) and 07Fh), 1 como entrada

    clrf TRISB ;puerto B como salida
    bsf TRISB, 4 ;puerto ((PORTB) and 07Fh), 4 como entrada
    bsf TRISB, 5 ;puerto ((PORTB) and 07Fh), 5 como entrada
...
config_reloj:
    banksel OSCCON
    bcf ((OSCCON) and 07Fh), 6 ;oscilador de 250 kHz (010)
